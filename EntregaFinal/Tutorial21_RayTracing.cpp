...
#include <random>
#include "../../../Samples/GLFWDemo/src/GLFWDemo.hpp"
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    SphereInstanceData SphereInstData[] = {
        {float3{8.0f, 8.0f, 0.0f}, 1.5f} // Posición base y offset temporal
    };
    ...
    const auto AnimateSphere = [&](TLASBuildInstanceData& Dst) {
        if (Dst.CustomId != 1) return; // Solo aplica a la esfera con CustomId = 1

        float  t     = sin(m_AnimationTime * PI_F * 0.5f + SphereInstData[0].TimeOffset);
        float3 Pos   = SphereInstData[0].BasePos + float3(sin(t * 1.5f), cos(t * 2.0f), sin(t * 0.8f)) * 3.0f;
        float  angle = 0.2f * PI_F * m_AnimationTime;
    ...
    Instances[3].Transform.SetRotation(float3x3::Identity().Data()); 
...
void Tutorial21_RayTracing::Update(double CurrTime, double ElapsedTime)
{
    ...
    UpdateUI();
    HandleKeyInput(static_cast<float>(ElapsedTime));
...
void Tutorial21_RayTracing::HandleKeyInput(float dt)
{
    float3 moveDir = float3(0, 0, 0);

    if (m_KeyLeft) moveDir.x -= 1.0f;
    if (m_KeyRight) moveDir.x += 1.0f;
    if (m_KeyUp) moveDir.z -= 1.0f;
    if (m_KeyDown) moveDir.z += 1.0f;

    if (moveDir.Length() > 0.0f)
    {
        moveDir = normalize(moveDir);
        m_SpherePosition += moveDir * m_SphereSpeed * dt;
    }
}
