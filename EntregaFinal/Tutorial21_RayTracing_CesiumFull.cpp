...
#include "Shaders/PBR/private/RenderPBR_Structures.fxh"
    ...
    void Tutorial21_RayTracing::CreateRayTracingPSO()
    {
        ...
        RefCntAutoPtr<IShader> pCubePrimaryHit, pGroundHit, pGlassPrimaryHit, pSpherePrimaryHit, pGLTFPrimaryHit;
        {
            ...
            ShaderCI.Desc.Name = "GLTF primary ray closest hit shader";
            ShaderCI.FilePath = "GLTFPrimaryHit.rchit";
            ShaderCI.EntryPoint = "main";
            m_pDevice->CreateShader(ShaderCI, &pGLTFPrimaryHit);
            VERIFY_EXPR(pGLTFPrimaryHit != nullptr);
        }
        ...
        PSOCreateInfo.AddTriangleHitShader("GLTFPrimaryHit", pGLTFPrimaryHit);
        ...
        ResourceLayout
            ...
            .AddVariable(SHADER_TYPE_RAY_CLOSEST_HIT, "g_GLTFTextures", SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE);
    ...
    void Tutorial21_RayTracing::LoadTextures()
    {
        ...
        for (int tex = 0; tex < NumTextures; ++tex)
        {
            ...
            Barriers[tex] = StateTransitionDesc{ pTex[tex], RESOURCE_STATE_UNKNOWN, RESOURCE_STATE_SHADER_RESOURCE, STATE_TRA
        ...
        m_pRayTracingSRB->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "g_GroundTexture")->Set(pGroundTex->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
                ...
                void Tutorial21_RayTracing::LoadGLTFModel()
            {
                GLTF::ModelCreateInfo ModelCI;
                ModelCI.FileName = "models/Oso/glTF/OSITO.gltf";
                ModelCI.ComputeBoundingBoxes = true;
                m_GLTFModel = std::make_unique<GLTF::Model>(m_pDevice, m_pImmediateContext, ModelCI);

                // Center and scale model
                m_GLTFModel->ComputeTransforms(m_GLTFModel->DefaultSceneId, m_GLTFTransforms);
                auto ModelAABB = m_GLTFModel->ComputeBoundingBox(m_GLTFModel->DefaultSceneId, m_GLTFTransforms);
                ...
                float SceneScale = (1.0f / std::max(MaxDim, 0.01f)) * 0.5f;
                auto Translate = -ModelAABB.Min - 0.5f * ModelDim;
                float4x4 InvYAxis = float4x4::Identity();
                InvYAxis._22 = -1;

                m_GLTFModelTransform = float4x4::Translation(float3(0, -3, 0)) * float4x4::Scale(SceneScale) * InvYAxis;
                m_GLTFModel->ComputeTransforms(m_GLTFModel->DefaultSceneId, m_GLTFTransforms, m_GLTFModelTransform);
            }
            ...
            void Tutorial21_RayTracing::CreateCubeBLAS()
            {
                ...
                CreateGeometryPrimitiveBuffers(m_pDevice, CubeGeometryPrimitiveAttributes{ CubeSize, GEOMETRY_PRIMITIVE_VERTEX_FLAG_POSITION }, &CubeBuffersCI, &pCubeVertexBuffer, &pCubeIndexBuffer);
            ...
            {
                std::vector<RefCntAutoPtr<IBuffer>> VertexBuffers;
                std::vector<RefCntAutoPtr<IBuffer>> IndexBuffers;
                ...
                for (const auto& mesh : m_GLTFModel->Meshes)
                {
                    for (const auto& primitive : mesh.Primitives)
                    {
                        ...
                        BufferDesc VtxBuffDesc;
                        VtxBuffDesc.Name = "GLTF Vertex Buffer";
                        VtxBuffDesc.Usage = USAGE_IMMUTABLE;
                        VtxBuffDesc.BindFlags = BIND_RAY_TRACING | BIND_VERTEX_BUFFER;
                        VtxBuffDesc.Size = primitive.pVertexBuffer->GetDesc().Size;
                        BufferData VtxData = { primitive.pVertexBuffer->GetDesc().pData, VtxBuffDesc.Size };
                        m_pDevice->CreateBuffer(VtxBuffDesc, &VtxData, &pVertexBuffer);

                        BufferDesc IdxBuffDesc;
                        IdxBuffDesc.Name = "GLTF Index Buffer";
                        IdxBuffDesc.Usage = USAGE_IMMUTABLE;
                        IdxBuffDesc.BindFlags = BIND_RAY_TRACING | BIND_INDEX_BUFFER;
                        IdxBuffDesc.Size = primitive.pIndexBuffer->GetDesc().Size;
                        BufferData IdxData = { primitive.pIndexBuffer->GetDesc().pData, IdxBuffDesc.Size };
                        m_pDevice->CreateBuffer(IdxBuffDesc, &IdxData, &pIndexBuffer);

                        BLASTriangleDesc Triangle;
                        Triangle.GeometryName = ("GLTFGeometry" + std::to_string(Triangles.size())).c_str();
                        Triangle.MaxVertexCount = primitive.VertexCount;
                        Triangle.VertexValueType = VT_FLOAT32;
                        Triangle.VertexComponentCount = 3;
                        Triangle.MaxPrimitiveCount = primitive.IndexCount / 3;
                        Triangle.IndexType = VT_UINT32;

                        BLASBuildTriangleData TriData;
                        TriData.GeometryName = Triangle.GeometryName;
                        TriData.pVertexBuffer = pVertexBuffer;
                        TriData.VertexStride = sizeof(float3);
                        TriData.VertexCount = Triangle.MaxVertexCount;
                        TriData.VertexValueType = Triangle.VertexValueType;
                        TriData.VertexComponentCount = Triangle.VertexComponentCount;
                        TriData.pIndexBuffer = pIndexBuffer;
                        TriData.PrimitiveCount = Triangle.MaxPrimitiveCount;
                        TriData.IndexType = Triangle.IndexType;
                ...
                ASDesc.Flags = RAYTRACING_BUILD_AS_PREFER_FAST_TRACE;
                ASDesc.pTriangles = Triangles.data();
                ASDesc.TriangleCount = static_cast<Uint32>(Triangles.size());
                ...
                Attribs.pTriangleData = TriangleData.data();
                Attribs.TriangleDataCount = static_cast<Uint32>(TriangleData.size());
                Attribs.pScratchBuffer = pScratchBuffer;
                Attribs.BLASTransitionMode = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
                Attribs.GeometryTransitionMode = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
                Attribs.ScratchBufferTransitionMode = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
                ...
                m_GLTFVertexBuffers = std::move(VertexBuffers);
                m_GLTFIndexBuffers = std::move(IndexBuffers);
            }

            void Tutorial21_RayTracing::UpdateTLAS()
            {
                static constexpr int BaseCount = 5; // Added GLTF model
                static constexpr int MaxTotal = BaseCount + MaxSmallSpheres;
                ...
                if (smallSpherePositions.empty())
                {
                    ...
                    std::uniform_real_distribution<float> distZ(-12.0f, 12.0f);
                    ...
                    for (int i = 0; i < MaxSmallSpheres; ++i)
                    {
                        float x = distX(rng);
                        float z = distZ(rng);
                        smallSpherePositions.emplace_back(x, yFloor, z);
                    }
                }
                ...
                Instances[1].InstanceName = "Sphere Instance";
                Instances[1].CustomId = 0;
                Instances[1].pBLAS = m_pProceduralBLAS;
                Instances[1].Mask = OPAQUE_GEOM_MASK;
                Instances[1].Transform.SetTranslation(0.0f, -100000000, 0.0f);

                Instances[2].InstanceName = "Sphere Instance 2";
                Instances[2].CustomId = 0;
                Instances[2].pBLAS = m_pProceduralBLAS;
                Instances[2].Mask = OPAQUE_GEOM_MASK;
                Instances[2].Transform.SetTranslation(-8.0f, -100000000, 0.0f);

                Instances[3].InstanceName = "Sphere Instance 3";
                Instances[3].CustomId = 0;
                Instances[3].pBLAS = m_pProceduralBLAS;
                Instances[3].Mask = OPAQUE_GEOM_MASK;
                Instances[3].Transform.SetTranslation(8.0f, -1000000, 0.0f);

                Instances[4].InstanceName = "GLTF OSITO Instance";
                Instances[4].pBLAS = m_pGLTFBLAS;
                Instances[4].Mask = OPAQUE_GEOM_MASK;
                Instances[4].Transform = m_GLTFModelTransform;
            ...
            void Tutorial21_RayTracing::CreateSBT()
            {
                ...
                m_pSBT->BindHitGroupForInstance(m_pTLAS, "GLTF OSITO Instance", PRIMARY_RAY_INDEX, "GLTFPrimaryHit");
                ...
                for (int i = 0; i < NumSmallSpheres; ++i)
                {
                    ...
                    m_pSBT->BindHitGroupForInstance(m_pTLAS, instanceName, PRIMARY_RAY_INDEX, "CubePrimaryHit");
                }
            ...
            void Tutorial21_RayTracing::UpdateUI()
            {
                ...
                {
                    ImGui::Separator();
                    ImGui::Text("Cajas de Dulces");
                    if (ImGui::SliderInt("Count", &m_NumScatterSpheres, 0, MaxSmallSpheres))
                    {
                        ...
                        for (int i = 0; i < m_NumScatterSpheres; ++i)
                        {
                            ...
                            m_ScatterSphereNames.push_back("ScatterSphere" + std::to_string(i));
                        }
                    }
                    ImGui::End();
                }
